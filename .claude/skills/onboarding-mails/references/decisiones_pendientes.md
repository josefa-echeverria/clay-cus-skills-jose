...
| 9 | ¿Se muestra comparación con la semana anterior en la tabla de avance (sección 1)? | **✅ Resuelto (ya se había acordado antes, pero no había quedado escrito en esta spec — septiembre 2026):** No. Se sacó la columna "Semana anterior" de la tabla de avance: el onboarder no tiene forma confiable de recuperar ese dato de un mail previo. Si esta decisión "se pierde" de nuevo, es señal de que hay que anotarla en algún lugar más persistente que la conversación — por ahora, esta tabla es la fuente de verdad. |
| 10 | ¿Se repite el bloque "Automatización con Cassius" cuando la empresa ya sale en el resumen agregado del grupo? | **✅ Resuelto (septiembre 2026):** No — es redundante. El bloque de Cassius (y desde esta misma decisión, también el punteo de tareas pendientes) solo se arma para empresas **sin** grupo (sección 2). Si la empresa tiene grupo, esos mismos 4 porcentajes y las tareas pendientes ya están en la fila de esa empresa dentro del resumen agregado (sección 3), que reemplaza por completo a la sección 2. |
| 11 | ¿Cómo se muestran las tareas pendientes de las **hijas** en el resumen agregado del grupo, si la card 6207 no trae sus filas? | **✅ Resuelto (septiembre 2026):** Piero creó la card **6230** ("Checklist de Grupo - Detalle por Empresa y Tarea (madre + hijas)", colección "Onboarding"), equivalente a la 6207 pero para hijas (excluye a la madre). Misma limitación de parámetros que 6301 — se ejecuta vía `execute_query`. Se muestran **solo las tareas en estado `Pendiente`**, como punteo (`<ul><li>`), nunca como tabla completa con las tareas `Ok` incluidas. |
| 12 | ¿Se mantiene la tabla completa de "próximos pasos" (todas las tareas, Ok y Pendiente) como sección aparte en los mails de seguimiento? | **✅ Resuelto (septiembre 2026):** No, se eliminó de los mails de seguimiento (2-8) — quedaba duplicada con el punteo de pendientes de la sección 2/3. **Se mantiene sin cambios en el Mail 1** (bienvenida), donde sigue teniendo sentido mostrar la agenda completa de las 11 tareas. |
| 13 | ¿Se mantiene la sección de "Sugerencias de uso" (product health / adopción por módulo) en los mails de seguimiento? | **✅ Resuelto (septiembre 2026):** No, se eliminó — Josefa indicó que no es necesaria. Si en el futuro se quiere retomar, la fuente seguía siendo `clay-dw:product-health` (top 3 módulos por `adopt_pct` más bajo). |
